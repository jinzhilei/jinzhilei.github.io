---
layout: page
title: Members
permalink: /members/
description: People who make up the Lei Lab.
nav: true
nav_order: 3
---

## Principal Investigator

<div class="card mt-3">
  <div class="p-3">
    <div class="row">
      <div class="col-sm-auto col-sm-3">
        <img class="img-fluid rounded" src="/assets/img/team/jinzhi_lei.jpg" alt="Jinzhi Lei" style="max-width:150px"/>
      </div>
      <div class="col">
        <h5 class="card-title">Jinzhi Lei (雷锦志)</h5>
        <h6 class="card-subtitle mb-2 text-muted">Professor · Doctoral Supervisor</h6>
        <p>
          Professor Lei received his B.S., M.S., and Ph.D. from Beihang University (Applied Mathematics / General Mechanics, 1991–2001),
          completed postdoctoral training at Tsinghua University (2001–2003), and was a research faculty member at
          Tsinghua University's Zhou Peiyuan Center for Applied Mathematics from 2003 to 2019.
          He joined Tiangong University in 2020. He has been a visiting assistant professor at UC Irvine (2004–2005)
          and a visiting scholar at McGill University (2005).
        </p>
        <p>
          His research focuses on computational systems biology, mathematical oncology, and dynamical systems.
          He has published over 90 peer-reviewed articles (17,000+ citations, Google Scholar)
          and authored three monographs on systems biology.
        </p>
        <p>
          📧 <a href="mailto:jzlei@tiangong.edu.cn">jzlei@tiangong.edu.cn</a> &nbsp;|&nbsp;
          <a href="https://scholar.google.com/citations?user=aR34e1gAAAAJ" target="_blank">Google Scholar</a> &nbsp;|&nbsp;
          <a href="https://orcid.org/0000-0002-2670-6760" target="_blank">ORCID</a> &nbsp;|&nbsp;
          <a href="https://github.com/jinzhilei" target="_blank">GitHub</a>
        </p>
      </div>
    </div>
  </div>
</div>

---

## PhD Students

*Current doctoral students are working on projects in cancer evolution dynamics, stem cell modelling, and single-cell data analysis.*

<!-- Add PhD students here as they join. Example format:
<div class="card mt-3">
  <div class="p-3">
    <h5>Student Name (姓名)</h5>
    <h6 class="text-muted">PhD Candidate · Enrolled YYYY</h6>
    <p>Research topic: ...</p>
    <p>📧 <a href="mailto:xxx@tiangong.edu.cn">xxx@tiangong.edu.cn</a></p>
  </div>
</div>
-->

---

## Master's Students

*Graduate students in the group carry out research at the intersection of mathematics, computation, and biology.*

<!-- Add master's students here. Same format as above. -->

---

## Alumni

| Name | Degree | Year | Current Position |
|------|--------|------|-----------------|
| *(to be filled)* | | | |

---

## Join Us

We welcome applications from students and researchers interested in **mathematical biology, computational systems biology, and related fields**. We particularly encourage applicants with strong backgrounds in:

- Applied mathematics (differential equations, dynamical systems, probability)
- Computational biology or bioinformatics
- Physics or engineering with interest in biological systems

**For prospective PhD / Master's students:** Tiangong University admits graduate students annually. Please contact Prof. Lei by email with your CV and a brief description of your research interests.

**For postdoctoral researchers:** Competitive positions may be available. Please send your CV, a research statement, and two reference letters to [jzlei@tiangong.edu.cn](mailto:jzlei@tiangong.edu.cn).
